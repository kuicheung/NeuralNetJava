...
public class DataSource implements Serializable{
	...
	private int[] testIndex = {90,76,64,54,43,33,21,16,3,0};
	...
	public DataSource(TreeMap<String,Profile> profiles,int inputCount,int idealCount,int[] jointIndex){
		...
		input = new double[profiles.size()-testIndex.length-verifyIndex.length][inputCount];
		ideal = new double[profiles.size()-testIndex.length-verifyIndex.length][idealCount];
		...
		for(String profileName : profiles.keySet()){
			...
			TreeMap<Integer,Double[]> scores = profile.getDTWScores();
			//build cells from scores and jointIndex
			double[] cells = new double[jointIndex.length*scores.get(1).length];
			int ct=0;
			for(int index : jointIndex){
				for(int i=0;i<scores.get(1).length;i++){
					cells[ct++]=scores.get(index)[i];
				}
			}
			double judgeScore = profile.getJudgeScore();	
			if(testSetIndex.contains(profileCt)){
				testInput[testCt] = ArrayUtils.clone(cells);
				testIdeal[testCt][0] = judgeScore;	
				testCt++;
			}
			else if(verifySetIndex.contains(profileCt)){
				verifyInput[verifyCt] = ArrayUtils.clone(cells);
				verifyIdeal[verifyCt][0] = judgeScore;	
				verifyCt++;
			}
			else{
				input[inputCt] = ArrayUtils.clone(cells);
				ideal[inputCt][0] = judgeScore;	
				inputCt++;
			}
			profileCt++;
		}
